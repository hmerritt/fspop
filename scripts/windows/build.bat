...
if x%2 == xFSPOP_DEV set _DEV_BUILD=1

cd %1
md bin 2>nul

:: Get the git commit
set _GIT_COMMIT_FILE=%TEMP%\fspop-git_commit.txt
set _GIT_DIRTY_FILE=%TEMP%\fspop-git_dirty.txt

set _NUL_CMP_FILE=%TEMP%\fspop-nul_cmp.txt
...
set _GO_ENV_TMP_FILE=%TEMP%\fspop-go-env.txt
